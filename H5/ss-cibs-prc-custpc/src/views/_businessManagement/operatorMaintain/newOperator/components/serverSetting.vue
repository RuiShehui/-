...
import { queryMenuAll } from '@/api/userManagement';
export default {
  props: {
    serverSettingObj: {
      required: true,
  ...
  data() {
    return {
      ...
      accSetList: [
        { accSet: this.$t('USER_MANAGEMENT.ADMINISTRATOR'), value: '1' },
        { accSet: this.$t('USER_MANAGEMENT.REPORTER'), value: '3' },
        { accSet: this.$t('USER_MANAGEMENT.AUTHORIZER'), value: '2' }
      ],
      tableData: [],
      originData: [],
  ...
  methods: {
    ...
    queryAllMenu() {
      let self = this;
      queryMenuAll().then(resp => {
        console.log('查询', resp.menuList);
        this.originData = resp.menuList;
    ...
    back() {
      this.$emit('back', this.serverSettingObj);
    },
