<template>
  <div class="g-deposit-set-time f-mt-24">
    ...
    >
      ...
      <!-- 账户名称 -->
      <el-table-column prop="accountName" width="280">
        <template #header>
      ...
      >
        <template slot-scope="scope">
          <div v-if="scope.row.currencyList">
            <span v-if="lang === 'en'">{{ scope.row.currency }}</span>
            <span v-else>{{ scope.row.currency | ccyFilter }}</span>
            {{ scope.row.accountType | accountType }}
          </div>

          <div v-else class="f-blue" @click="toAccountDetial(scope.row)">
            -
            <span v-if="lang === 'en'">{{ scope.row.currency }}</span>
            <span v-else>{{ scope.row.currency | ccyFilter }}</span>
          </div>
      ...
      >
        <template slot-scope="scope">
          <span>{{ formatMoney('',scope.row.accountBalance) }}</span>
        </template>
      </el-table-column>
      <!-- 状态 -->
