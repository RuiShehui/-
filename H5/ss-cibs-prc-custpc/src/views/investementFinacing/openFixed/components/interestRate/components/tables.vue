 ...
 * @LastEditTime: 2022-09-19 10:16:28
    ...
    <div class="content" v-for="(item,index) in tableData" :key="index">
      ...
      <div class="rate">
        <div v-for="(itm,idx) in item" :key="idx">
          <div class="money-num">
            <span v-if="idx > 0 && idx < item.length - 1">{{ toInt(item[idx-1].AMOUNT,itm.CCY) }} {{$t('ACCOUNT_MANAGEMENT.DATESPAN_TO')}}</span>
            <span v-if="idx === 0 && itm.CCY !== 'JPY'">0.00 {{$t('ACCOUNT_MANAGEMENT.DATESPAN_TO')}}</span>
            <span v-if="idx === 0 && itm.CCY === 'JPY'">0 {{$t('ACCOUNT_MANAGEMENT.DATESPAN_TO')}}</span>
            <span v-if="idx === item.length - 1">{{ toInt(item[idx-1].AMOUNT,itm.CCY) }} {{$t('INTERESTRATEQUERY.ABOVE')}}</span>
