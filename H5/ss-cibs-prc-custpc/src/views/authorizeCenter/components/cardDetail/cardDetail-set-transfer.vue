  ...
  <div class="g-card-detail-wrapper">
    ...
    <div class="g-card-detail-wrapper__middle">
      <div class="u-content">
        ...
        <div class="u-content__item">
          <div class="u-content__item__left">{{ $t("AUTHORIZE.AUTH_STATUS") }}</div>
          <div class="u-content__item__right">{{smallTransInfo.setStatus === '1' ? this.$t('USER_MANAGEMENT.ENABLE') : this.$t('USER_MANAGEMENT.DEACTIVATE')}}</div>
        </div>
        <div class="u-content__item">
          <div class="u-content__item__left">{{ $t("AUTHORIZE.AUTH_DAYLIMIT") }}</div>
          <div class="u-content__item__right">HKD {{smallTransInfo.limitAmount | formatMoneyNew}}</div>
        </div>
      </div>
    </div>
    <div v-if="isWaitAuth">
...
<script>
import {
  setUpSmallTransferAuth,
  setUpSmallTransferDetails
...
export default {
  ...
  methods: {
    ...
    submitStatus() {
      ...
      setUpSmallTransferAuth(params).then(resp => {
        // console.log('打印收取按', resp);
        this.$router.push({
    ...
    //查询详情
    queryDetail() {
      setUpSmallTransferDetails({ commFlow: this.tableData.flowNo }).then(
        resp => {
          this.smallTransInfo = JSON.parse(resp.tradeDetail);
          console.log('打印resp小额', JSON.parse(resp.tradeDetail));
        }
      );
    },
    //返回
