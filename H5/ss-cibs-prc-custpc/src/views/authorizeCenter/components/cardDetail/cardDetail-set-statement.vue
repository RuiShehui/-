    ...
    <div class="g-card-detail-wrapper__middle">
      <div class="u-content">
        ...
        <div class="u-content__item">
          ...
          <!-- <div class="u-content__item__right">设定电子结单</div> -->
      ...
      <div style="margin-bottom: 24px">
        <el-table :data="tableDataArr" stripe>
          <el-table-column min-width="30%" :label="$t('AUTHORIZE.ACCOUNT')" prop="accountNameNo"></el-table-column>
          <el-table-column min-width="30%" :label="$t('AUTHORIZE.debit_AcctName')" prop="accountName"></el-table-column>
          <el-table-column min-width="25%" :label="$t('AUTHORIZE.billTypeInfo')" prop="stateType"></el-table-column>
          <el-table-column min-width="15%" :label="$t('AUTHORIZE.AUTH_STATEMENTS')" prop="stateStatusName">
            <!-- <template slot-scope="scope">
              <el-switch v-model="scope.row.stateStatus"></el-switch>
            </template>-->
          </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    submitStatus() {
      ...
      updateElecStateAuth(params).then(resp => {
    ...
    //详情查询
    queryDetail() {
      updateElecStateQuery({ flowNo: this.tableData.flowNo }).then(resp => {
        console.log('电子结单resp', resp);
        if (resp.statements.length > 0) {
          resp.statements.forEach(item => {
            ...
            item.accountNameNo =
              item.accountNo + ' ' + item.currency + ' ' + this.$options.filters['accountType'](item.accountType);
            if (item.stateStatus === '0') {
              item.stateStatusName = this.$t('USER_MANAGEMENT.DEACTIVATE');
            } else {
              item.stateStatusName = this.$t('USER_MANAGEMENT.ENABLE');
            }
            if (item.stateType === 'MONTHLY_STATEMENT') {
              item.stateType = this.$t('AUTHORIZE.AUTH_MONTHLY');
            }
            if (item.stateType === 'REGULAR_BASIS_STATEMENT') {
              item.stateType = this.$t('AUTHORIZE.AUTH_NOTICE');
            }
          });
        }
        this.tableDataArr = resp.statements;
