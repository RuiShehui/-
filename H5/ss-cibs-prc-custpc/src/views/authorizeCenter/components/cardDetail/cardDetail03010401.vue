  ...
  <div class="g-card-detail-wrapper">
    ...
    <div class="g-card-detail-wrapper__middle">
      <div class="u-content">
        ...
        <div class="u-content__item" v-if="detailData.isScheduleTx === 'Y'">
          <!-- 预约处理时间 -->
          <div class="u-content__item__left">{{$t('AUTHORIZE.scheduleDate')}}</div>
          <div class="u-content__item__right">{{ dataDep.lastScheduleDate }}</div>
        </div>
        <div class="u-content__item">
          <!-- 提交时间 -->
      ...
      <div class="m-table" v-if="accountList.length > 0">
        <el-table :data="accountList.slice((currentPage-1)*pageSize,currentPage*pageSize)" stripe>
          ...
          <el-table-column prop="amount" :label="$t(`PAYMENT.TRANSFER_AMOUNT`)" width="165">
...
import { auth, transferAuth } from '@/api/cashManage';
...
export default {
    ...
    opTypeFilter(n) {
      if (n) {
        return `${that.$t('AUTHORIZE.opType')}-${that.$t(
          'AUTHORIZE.ALL_TRANSFER'
        )}`;
      } else {
        // 批量转账
        return that.$t('AUTHORIZE.ALL_TRANSFER');
      }
    }
  },
  data() {
  ...
  created() {
    ...
    if (transactionTypes == '0303') {
      ...
      this.transactionType = '03010401';
    }
    this.detailData = this.dataDep;
  },
  methods: {
    //授权接口
    async authInit() {
      ...
      const result = await transferAuth(params);
