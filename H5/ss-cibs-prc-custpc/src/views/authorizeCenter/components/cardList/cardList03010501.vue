 ...
 * @LastEditTime: 2022-09-24 15:25:40
 * @LastEditors: Please set LastEditors
 * @Description: 批量发薪
 * @FilePath: \ss-cibs-prc-custpc\src\views\authorizeCenter\components\cardList\cardList03010501.vue
-->
<template>
<!-- 批量发薪 -->
  ...
  methods:{
     filterType(){
      if(this.item.businessCode==='0303'){
        this.title = this.$t('AUTHORIZE.APPOINT_TRANS_MANAGE')+'-'+this.$t('AUTHORIZE.ALL_NEWTRANSTER')
      }else{
        this.title = this.$t('AUTHORIZE.ALL_NEWTRANSTER')
      }
    },
     formatAccountNo(val) {
