  ...
  methods: {
    ...
    async handleUploadFile2(item) {
      return new Promise(async (resolve, reject) => {
        ...
        formData.type = 'BATCH_PAYOFF'; // BATCH_PAYOFF
    ...
    async saveTransfer(params = {}) {
      ...
      const { body } = await addTransactionDraft({
        ...
        transactionType: '6', //交易类型：1-行内转账；2-转数快；3-CHATS；4-电汇；5-批量转账；6-批量发薪；7-自动转账收款
    ...
    transConfirm() {
      ...
      if (formValidState) {
        ...
        let params = {
          ...
          tradeType: '01' //交易类型,01:批量发薪,02:批量转账,03:自动转账收款s
        };
        //批量转账/发薪校验行内账户
