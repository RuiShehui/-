<!--
 * @Author: ZhangZhen
 * @Date: 2021-12-07 16:19:41
 * @LastEditTime: 2022-07-06 19:57:22
 * @LastEditors: zhangcheng
 * @Description: Zz's design
 * @FilePath: \ss-cibs-prc-inmpc\nodejs_cibs-prc-inmpc\src\components\money-input\index.vue
-->
<template>
  <el-input
    :class="['f-w-150']"
    :value="moneyValue"
    :size="size"
    :placeholder="placeholder"
    :maxlength="maxlength"
    :disabled="disabled"
    @input="onMoneyInput"
    @change.native="onChange"
    @blur="onBlur"
    @focus="onFocus"
    @copy.native.capture.prevent="() => false"
    @cut.native.capture.prevent="() => false"
    @paste.native.capture.prevent="() => false"
  />
</template>

<script>
export default {
  name: 'MoneyInput',
  model: {
    prop: 'moneyValue',
    event: 'onMoneyInput'
  },
  props: {
    /** 金额输入框的原始值  */
    moneyValue: {
      type: String,
      default: ''
    },
    /** 金额输入框的类型  */
    type: {
      type: String
      // validator(val) {
      //   return ['text', 'textarea', 'tel', 'email', 'daterange', undefined].includes(val);
      // },
    },
    /** 金额输入框的最大长度  */
    maxlength: {
      type: [Number, String],
      default: 17
    },
    size: String,
    /** placeholder */
    placeholder: {
      type: String,
      default() {
        return this.$t('COMMON.PLEASE_INPUT');
      }
    },
    isThousand: {
      type: Boolean,
      default: false
    },
    // 是否自动添加小数点
    isSaveDot: {
      type: Boolean,
      default: false
    },
    // 限制只能输入整数（不能00开头）
    isFullNumber: {
      type: Boolean,
      default: false
    },
    /** 是否禁填 */
    disabled: Boolean,
    /** 是否保留整数 */
    isAllNum: {
      type: Boolean,
      default: false
    }
  },
  data() {
    return {
      // 输入的金额值
      inputMoneyArray: []
    };
  },
  methods: {
    /**
     * handle money-input change
     */
    onMoneyInput(value) {
      const filterInput = this.filterInput(value);

      this.$emit('onMoneyInput', filterInput);
    },
    onChange(e) {
      this.$emit('change', e);
    },
    /** filter-input-value */
    filterInput(value) {
      let _val = '';

      const valueList = value.split('');
      /*
        1. amount[0] === '0' && amount[1] !== '.'
        2. '.' -> '0.'
        3. /^\d*(\.?\d{0,2})/g
      */
      if (this.isAllNum) {
        return this.handleAllNumInput(value);
      }
      if (valueList[0] === '0' && valueList[1] !== '.') {
        _val = '0';
        return _val;
      }
      if (this.isFullNumber) {
        _val = value.match(/^\d*/g)[0];
      } else {
        _val = this.handleCommonNumInput(value);
      }
      return _val;
    },
    /**
     * 普通输入限制
     */
    handleCommonNumInput(value) {
      const valueList = value.split('');

      const validMoney = this.isSaveDot
        ? new RegExp(`\\d{0,${this.maxlength - 3}}(\\.\\d{0,2})?`, 'g')
        : /\d{0,14}/g;

      if (valueList[0] === '.') {
        return '0.';
      }

      if (valueList[0] === '.') {
        return '0.';
      }

      return value.match(validMoney)[0];
    },
    /**
     * 整数输入限制
     */
    handleAllNumInput(value = '') {
      return value.match(/^\d*/g)[0];
    },
    /** onfocus */
    onFocus(e) {
      this.$emit('focus', e);
    },
    /** on-blur */
    onBlur(e) {
      if (this.isSaveDot) {
        const inputVal = e.target.value;
        if (!inputVal.length) {
          return;
        }
        const inputValArr = inputVal.split('.');
        if (!inputValArr[1]) {
          inputValArr[1] = '00';
        }
        if (inputValArr[1] && inputValArr[1].length === 1) {
          inputValArr[1] = inputValArr[1] + '0';
        }
        if (this.isThousand) {
          inputValArr[0] = inputValArr[0]
            .split('')
            .reverse()
            .reduce((pre, next, index) => {
              return (index % 3 ? next : next + ',') + pre;
            });
        }
        const filterInput = inputValArr.join('.');
        e.target.value = filterInput;
        this.$emit('changeBlur', filterInput);
      } else {
        if (this.isThousand) {
          inputVal = inputVal
            .split('')
            .reverse()
            .reduce((pre, next, index) => {
              return (index % 3 ? next : next + ',') + pre;
            });
        }
        const inputVal = e.target.value;
        e.target.value = inputVal;
        this.$emit('changeBlur', inputVal);
      }
    }
  }
};
</script>

<style scoped lang="scss">
.f-w-150 {
  width: 150px;
}
</style>
