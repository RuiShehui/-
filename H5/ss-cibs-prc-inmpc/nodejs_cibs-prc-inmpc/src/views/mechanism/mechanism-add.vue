 ...
 * @LastEditTime: 2022-01-27 09:46:19
 * @Description: 
 * @FilePath: \ss-cibs-prc-inmpc\nodejs_cibs-prc-inmpc\src\views\mechanism\mechanism-add.vue
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      //   url: "/mechanism/add"
      // };
      // self.$router.push({ path: "/mechanism/submit", query: obj });
    },
    ...
    confirm(formName) {
      let self = this;
      saveOrganizationInfo(self.ruleForm).then(resp => {
        console.log('打印新增机构', resp);
        if (resp.body) {
          // self.$showMsg.success('操作成功！');
          // self.$showMsg.success(self.$t('GROUP_MANAGEMENT.ADD_GROUP_SUCCESS'));
          self.$showMsg({
            type: 'success',
            message: self.$t('GROUP_MANAGEMENT.ADD_GROUP_SUCCESS')
          });
          self.cancel();
        } else {
