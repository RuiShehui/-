 ...
 * @LastEditTime: 2022-06-09 17:59:40
 * @Description: 
 * @FilePath: \ss-cibs-prc-inmpc\nodejs_cibs-prc-inmpc\src\views\system-manage\components\user\UserManageAdd.vue
  ...
  <div class="contain">
    <header class="g-form">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="calc(50% - 170px)">
        ...
        <el-form-item  :label="$t(`USER_MANAGEMENT.USER_OWNING_REGION`)">
   ...
   submitForm() {
      this.$refs.ruleForm.validate(async  valid => {
        if (valid) {
          ...
          const res=await this.processingData(this.ruleForm);
        } else {
          return false;
        }
      });
    ...
    processingData(ruleForm) {
      ...
      this.addRole();
      console.log('打印处理后的对象数据：', this.userObject);
    },
    ...
    //确认
    confirm(formName) {
      addUserInfo(this.userObject).then(resp => {
        // console.log('打印新增resp', resp);
        if (resp.body) {
          this.$showMsg({
            type: 'success',
            message: this.$t('USER_MANAGEMENT.ADD_USER_SUCCESS')
          });
          this.cancel();
        } else {
          // this.$showMsg.error('新增失败！');
        }
      });
    },
