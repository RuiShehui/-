 ...
 * @LastEditTime: 2022-02-11 15:54:33
 * @LastEditors: zhangcheng
 * @Description: In User Settings Edit
 * @FilePath: \ss-cibs-prc-inmpc\nodejs_cibs-prc-inmpc\src\views\system-manage\components\role\RoleManageAdd.vue
-->
<template>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      if (!this.roleName) {
        // return this.$showMsg.error('请输入角色名称');
        return this.$showMsg({
      ...
      if (
        ...
        // return this.$showMsg.error('请至少选择一个角色权限');
        return this.$showMsg({
      ...
      if ((getRoleDetail().roleName == this.roleName) ) {
    ...
    addRequest(params) {
      roleAdd(params).then(resp => {
        ...
        if (resp.body) {
          ...
          // this.$showMsg.success(this.$t('ROLE_MANAGEMENT.ADD_ROLE_SUCCESS'));
          this.$showMsg({
            type: 'success',
            message: this.$t('ROLE_MANAGEMENT.ADD_ROLE_SUCCESS')
          });
          this.cancel();
        } else {
