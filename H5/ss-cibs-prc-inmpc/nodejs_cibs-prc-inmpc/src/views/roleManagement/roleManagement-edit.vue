 ...
 * @LastEditTime: 2022-01-14 10:09:34
 * @LastEditors: ZhangZhen
 * @Description: In User Settings Edit
 * @FilePath: \ss-cibs-prc-inmpc\nodejs_cibs-prc-inmpc\src\views\roleManagement\roleManagement-edit.vue
-->
<template>
  <div class="contain">
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      let self = this;
      if (!self.roleName) {
        // return self.$showMsg.error('新增角色不能为空！');
        return self.$showMsg.error(
          self.$t('ROLE_MANAGEMENT.PLEASE_INPUT_ROLE_NAME')
        );
      }
      if (
        ...
        // return self.$showMsg.error('新增菜单选择不能为空！');
        return self.$showMsg.error(
          self.$t('ROLE_MANAGEMENT.PLEASE_SELECT_MENU')
        );
      }
      // if (
      //   getRoleDetail().roleName === self.roleName &&
      //   self.roleName === '管理员'
      // ) {
      //   // return this.$showMsg.success('管理员无法添加管理员角色！');
      //   return self.$showMsg.error(
      //     self.$t('ROLE_MANAGEMENT.ADMIN_NO_ADD_ADMIN')
      //   );
      // }
      self.isSubmit = true;
    ...
    updateRequest(params) {
      ...
      updateRuleInfo(params).then(resp => {
        if (resp.body) {
          self.$showMsg({
            type: 'success',
            message: self.$t('ROLE_MANAGEMENT.EDIT_ROLE_SUCCESS')
          });
          self.cancel();
          // }
        }
